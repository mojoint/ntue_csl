...
(function($) {
  $(function() {
    ...
    if (mojo.mojo != "") {
        ...
        mojo.classes = $('#academic_agency_class').attr('data-classes');
              ...
              case 'academic_agency':
                ...
                case 'del':
                  ...
                  $('.btn-academic_agency-mod').on('click', function(e) {
                      ...
                      var tds = $(tr).find("td");
                      mojo.dialog_maintain( 'academic_agency', 'mod', {"id": $(tds[0]).html(), "institution_code": $(tds[1]).html(), "cname": $(tds[3]).html(), "agency_state": $(tds[9]).html()} );
                  });
...
console.log( res.data );
              ...
              case 'academic_agency_report':
                ...
                case 'search':
                  if (res.data.summary) {
                    ...
                    html_a += '<th>' + mojo.summary.a.avg_weekly.toFixed(2) + '</th>';
                    ...
                    html_b += '<th>' + mojo.summary.b.avg_weekly.toFixed(2) + '</th>';
                    ...
                    html_c += '<th>' + mojo.summary.c.avg_weekly.toFixed(2) + '</th>';
                    ...
                    mojo.summary.s.new_people = mojo.summary.a.new_people + mojo.summary.b.new_people + mojo.summary.c.new_people;
                    mojo.summary.s.people = mojo.summary.a.people + mojo.summary.b.people + mojo.summary.c.people;
                    mojo.summary.s.weekly = mojo.summary.a.weekly + mojo.summary.b.weekly + mojo.summary.c.weekly;
                    mojo.summary.s.avg_weekly = mojo.summary.a.avg_weekly + mojo.summary.b.avg_weekly + mojo.summary.c.avg_weekly;
                    mojo.summary.s.hours = mojo.summary.a.hours + mojo.summary.b.hours + mojo.summary.c.hours;
                    mojo.summary.s.total_hours = mojo.summary.a.total_hours + mojo.summary.b.total_hours + mojo.summary.c.total_hours;
                    mojo.summary.s.turnover = mojo.summary.a.turnover + mojo.summary.b.turnover + mojo.summary.c.turnover;
                    mojo.summary.s.classes = mojo.summary.a.classes + mojo.summary.b.classes + mojo.summary.c.classes;

                    html_s += '<tr class="active">';
                    html_s += '<th >三研習類合計</th>';
                    html_s += '<th>' + mojo.summary.s.new_people + '</th>';
                    html_s += '<th>' + mojo.summary.s.people + '</th>';
                    html_s += '<th>' + mojo.summary.s.weekly + '</th>';
                    html_s += '<th>' + mojo.summary.s.avg_weekly.toFixed(2) + '</th>';
                    html_s += '<th>' + mojo.summary.s.hours + '</th>';
                    html_s += '<th>' + mojo.summary.s.total_hours + '</th>';
                    html_s += '<th>' + mojo.summary.s.turnover + '</th>';
                    html_s += '<th>' + mojo.summary.s.classes + '</th>';
                    html_s += '<th></th><th></th>';
                    html_s += '</tr>';
                  ...
                  if (res.data.detail) {
                    ...
                    for (var i=0; i<mojo.data.academic_agency_report_detail_a.length; i++) {
                      for (var j=0, jsize=mojo.data.academic_agency_report_detail_a[i]['country'].length-1; j<mojo.data.academic_agency_report_detail_a[i]['country'].length; j++) {
                        ...
                        if (j == jsize)
                          html_a += mojo.quarters[ mojo.data.academic_agency_report_detail_a[i]['quarter'] ] + ':' + mojo.data.academic_agency_report_detail_a[i]['minor_code_cname'];
                        html_a += '</td>';
                    ...
                    for (var i=0; i<mojo.data.academic_agency_report_detail_b.length; i++) {
                      for (var j=0, jsize=mojo.data.academic_agency_report_detail_b[i]['country'].length-1; j<mojo.data.academic_agency_report_detail_b[i]['country'].length; j++) {
                        ...
                        if (j == jsize)
                          html_b += mojo.quarters[ mojo.data.academic_agency_report_detail_b[i]['quarter'] ] + ':' + mojo.data.academic_agency_report_detail_b[i]['minor_code_cname'];
                        html_b += '</td>';
                    ...
                    for (var i=0; i<mojo.data.academic_agency_report_detail_c.length; i++) {
                      for (var j=0, jsize=mojo.data.academic_agency_report_detail_c[i]['country'].length-1; j<mojo.data.academic_agency_report_detail_c[i]['country'].length; j++) {
                        ...
                        if (j == jsize)
                          html_c += mojo.quarters[ mojo.data.academic_agency_report_detail_c[i]['quarter'] ] + ':' + mojo.data.academic_agency_report_detail_c[i]['minor_code_cname'];
                        html_c += '</td>';
    ...
    */
    /* =========================================== */
    /* ----------------- int, float ----------------- */
    mojo.input_check = function() {
        var orig = 0;
        var curr = 0;
        $('.input-int').on('focus', function(e) {
          orig = $(this).val(); 
          $(this).val("");
        }).on('blur', function(e) {
          curr = $(this).val();
console.log( orig );
console.log( isNaN(parseInt( orig )) );
console.log( curr );
console.log( isNaN(parseInt( curr )) );
          if (orig != curr) {
            if (isNaN(parseInt(curr))) {
              if (isNaN(parseInt(orig)))
                $(this).val(0);
              else 
                $(this).val(parseInt(orig));
            } else {
                $(this).val(parseInt(curr));
            } 
          } 
        });

        $('.input-float').on('focus', function(e) {
          orig = $(this).val(); 
          $(this).val("");
        }).on('blur', function(e) {
          curr = $(this).val();
          if (orig != curr) {
            if (isNaN(parseFloat(curr))) {
              if (isNaN(parseFloat(orig)))
                $(this).val(0);
              else 
                $(this).val(parseFloat(orig));
            } else {
                $(this).val(parseFloat(curr));
            } 
          }
        });
    };
    ...
    mojo.dialog_maintain = function(key, val, params) {
      ...
      case 'academic_agency':
          ...
          actions: [
            { text: '確定', primary: true, action: function(e) {
              ...
              case 'del':
                ...
                break;
              case 'mod':
                mojo.json = { 'id': params.id, 'institution_code': $('#dialog-institution_code').val(), 'cname': $('#dialog-cname').val(), 'agency_state': ($('#dialog-agency_state').is(":checked"))? "1" : "0" };
                break;
              }
        ...
        case 'mod':
          ...
          mojo.html += '<div class="k-textbox k-textbox-full k-space-right"><label for="dialog-cname">機構名稱</label><input type="text" id="dialog-cname" class="form-control" /></div>';
          mojo.html += '<div><input type="checkbox" id="dialog-agency_state" class="form-control mini-chkbox" /><label for="dialog-agency_state">舊生需求</label></div>';
          ...
          if (parseInt(params.agency_state)) 
            $('#dialog-agency_state').prop("checked", true);
    ...
    mojo.watch_admin_report = function() {
      ...
      $('#btn-academic_admin_report_manager-export').on('click', function(e) {
        e.preventDefault();
        window.open('/ajax/reporter/academic_admin_report/manager/' + $('#academic_admin_report_manager-era').val() + '/' + $('#academic_admin_report_manager-quarters').val());
      });
      ...
      $('#btn-academic_admin_report_special-export').on('click', function(e) {
        e.preventDefault();
console.log( Base64.encode( $('#academic_admin_report_special-sql').val() ) );
        window.open('/ajax/reporter/academic_admin_report/special/' + $('#academic_admin_report_special-era').val() + '/' + Base64.encode( $('#academic_admin_report_special-sql').val() ));
      });
    ...
    mojo.watch_fill = function() {
      ...
      mojo.data.academic_agency_class_summary = [{'cname': '研習類別總計', 'new_people': 0, 'people': 0, 'total_hours': 0, 'turnover': 0}];
      ...
      for (var i=0; i<mojo.data.academic_agency_class.length; i++) {
        ...
        mojo.data.academic_agency_class_summary[0]['new_people'] += mojo.data.academic_agency_class[i]['new_people'];
        mojo.data.academic_agency_class_summary[0]['people'] += mojo.data.academic_agency_class[i]['people'];
        mojo.data.academic_agency_class_summary[0]['total_hours'] += mojo.data.academic_agency_class[i]['total_hours'];
        mojo.data.academic_agency_class_summary[0]['turnover'] += mojo.data.academic_agency_class[i]['turnover'];
      }
      ...
      mojo.grid.academic_agency_class_summary.kendoGrid({
        data: mojo.data.academic_agency_class_summary,
        pageable: false,
        columns: [
